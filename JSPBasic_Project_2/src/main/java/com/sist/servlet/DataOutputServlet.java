...
public class DataOutputServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 사용자가 보내준 값을 받는다
		/*
		 * 	   사용자가 전송 ==> 모든 전송된 데이터는 request안에 있다 => 구분자 : name 
		 * 	   requset => HttpServletRequest
		 * 	    	      ==================
		 *                 javax.servlet.http.HttpServletRequset
		 *                   => tomcat 9
		 *                 jakarata.servlet.http.HttpServletRequset
		 *                   => tomcat 10 
		 *     1) 서버정보
		 *        http://localhost:80
		 *        ===================  서버 IP / 서버 PORT
		 *        		=> gerSercerInfo() / getPort()
		 *     2) 사용자 전송 정보
		...
		out.write("</ul>");
		out.write("a href=\"DataInputServlert\">입력</a>"); //\"URL주소\"
		out.write("<hr>");
		...
		out.write("request는 화면을 이동 / 새로고침시에 초기화 된다");
		out.write("</body>");
		out.write("</html>");
