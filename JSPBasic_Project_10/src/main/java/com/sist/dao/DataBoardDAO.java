...
// DBCP => 보안 / Connection을 효율적으로 관리(POOL) / 시간연결에 소모되는 시간 절약
// 한번에 접속자가 많이 있어도 쉽게 (서버)다운이 안된다
// DBCP => 웹 프로그램 일반화 (실무는 거의 대부분 => 98%) => 홈페이지 (일반 JDBC) , 공기업 / 금융권 / 포탈 (DBCP) 
...
public class DataBoardDAO {
	...
	private PreparedStatement ps; // SQL문장 송수신 ~> 오라클과 송수신 
	...
	// static => 메모리 공간을 1개만 설정이 가능 
	// 미리 연결해서 저장된 POOL안에 Connection 주솔르 얻어온다 
	...
	public void disConnection()
	{
		try {
			if(ps!=null)
				ps.close();
			if(conn!=null)
				conn.close(); // COnnection반환이 된다 => commons-dbcp.jar (Apache)
			//                                        commons-pool.jar 
			// => tomcat => lib => 저장
		}catch(Exception ex)
		{
	...
	// 나머지는 동일 
	// 목록 => 페이징
	public List<DataBoardVO> databoardListData(int page) // 사용자에게 page를 받음
	{
	...
	// 데이터가 삭제 전에 파일 정보 읽기 => 파일을 폴더에서 삭제 가능 
