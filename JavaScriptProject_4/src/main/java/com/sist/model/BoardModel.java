...
public class BoardModel {
	 ...
	 * 		display(10, 10)
	 */
	// 메소드 => Controller => 모든 메소드의 매개변수 request, response 
	...
	{	try {
		...
		return "redirect:list.do"; // request 초기화 
	}
	@RequestMapping("board/detail.do") // => if를 추가 => 찾기 => 기능은 없다 
	...
	@RequestMapping("boadr/delete.do")
	...
	{
	//     void : 화면이동 X => javascript에서 처리 : Spring
		String name=request.getParameter("name");
		String subject=request.getParameter("subject");
		String content=request.getParameter("content");
		String pwd=request.getParameter("pwd");
