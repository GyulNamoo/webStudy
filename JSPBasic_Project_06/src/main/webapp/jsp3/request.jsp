 ...
 <%--
 		 ...
 		 ***HttpServletRequest request 
 		 ...
 		 ================================================
 		  ...
 		  = ASP, PHP	
 		       1. 서버 정보 / 브라우저 정보 
 		       	  http://localhost:80  /JSPBasic_Project_06/jsp3/request.jsp => URL
 		       	  ================     ==================================== URI
 		       	  					   ====================            
 		       	  					         ContextPath
 		       	  					         => 구분자가 없다 : 파일명자체가 구분자 
 		       	       서버IP/Port => 80은 생략이 가능                 
 		       	  	   ...
 		       	  	   => getProtocol() : 약속 => http 
 		       	  	   => getPost() : 80
 		       	  	                      => getRequestURL()    http://localhost:80  /JSPBasic_Project_06/jsp3/request.jsp
 		       	  	                      => ***getRequestURI()    /JSPBasic_Project_06/jsp3/
 		       	  	                      => ***getContextPath()   /JSPBasic_Project_06
...
<%--
	사용자가 전송하는 모든 데이터는 서버를 거쳐서 톰캣에 의해 request에 전송된 데이터가 묶여서 넘겨준다 
										  =============================
										  	=> 처리를 웹개발자가 한다
 	묶는 방법 => Map 방식이다 
 	          ====== 키,값 쌍으로...!! => getParameter("키")
 	          list.jsp?1&num1=2&num3=3&4&5
 	 setAttribute(키,값) : 키는 중복이 불가능하다 
 	 
 	 => getRequestURI()
 	    => /프로젝트 폴더명/폴더명/파일명?id=admin&pwd=1234
 	                           ================== request에 첨부 (주소가 아니다/데이터전송!) 
 	       ====================URI(실제경로)
		  ...
		  <form method="post" action="request_ok.jsp">
			<table calss="table">
				...
				<tr>
					<th width=20%>소개</th>
					<td width=80%>
					 <textarea rows="5" cols="30" name="center"></textarea>
					 </td>
				</tr>
				<tr>
					<th width=20%>소개</th>
					<td width=80%>
					 ...
					 <input type="checkbox" name=hobby value="여향">여행
