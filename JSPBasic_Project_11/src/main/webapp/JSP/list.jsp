...
<%--
	 데이터 관리 => 지속적으로 상태 관리
	  ...
	  ==================================================
	  저장 장소       클라이언트 브라우저           sessionId외 다른 실제 데이터는 서버에 저장 
	  				   				     ========== 브라우저 구분자 (사용자의IP나 PORT)
	  ...
	  Cookie의 주요 메소드   
	    ...
	    ***5. 값 읽기 
	       cookie.getValue()
	    6. 삭제 방법
	       cookie.setMaxAge(0) => addCoockie를 이용해서 브라우저로 전송
	    7. 전체 쿠키를 읽을 경우  
	  ...
	  Session의 주요 메소드 => 서버에 저장 (구분 => getId(), 저장 공간 클라이언트당 한개만 배정)
	    ***1. 생성
...
<%
	...
	SeoulDAO dao = SeoulDAO.newInstance();
	 ...
	 if(cookies!=null)
	 {
		 ...
		 {
			 if(cookies[i].getName().startWith("seoul_"))
			 {
