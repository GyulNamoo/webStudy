		 ...
		 type:'post',
	 		...
	 		success:function(json)
	 		{
	 			...
	 			json.map(function(reply){
	 				...
	 				html+='<tr>'
	 				html+='<td class="text-legft">'◐+reply.name+'('+reply.dbday+')'
	 				html+='<td class="text-right">'
	 				if(reply.id===reply.sessionId)
	 				{
	 					html+='<span class="btn btn-xs btn-success ups" onclick="replyUpdate('+reply.rno+')" data-no="'+reply.rno+'">수정</span>'
	 					html+='<input type="button" class="btn btn-xs btn-warning" value="삭제" onclick="'+replyDelete(reply.bno)+'">'
	 				}
	 				html+='</td>'
	 				html+='</tr>'
	 				html+='<tr>'
				...
				<table class="table">
					...
					<c:if test="${vo.filesize>0}">
						<tr>
							...
							<td colspan="3"><a
								href="../databoard/download.do?fn=${vo.filename }">${vo.filename }</a>(${vo.filesize}Bytes)</td>
						</tr>
					</c:if>
					<%--
        ...
        --%>
					...
					<tr>
						<td colspan="4" class="text-right"><a
							href="../databoard/update.do?no=${vo.no }"
							class="btn btn-xs btn-success">수정</a> <span
							class="btn btn-xs btn-warning" id="delBtn">삭제</span> <a
							href="../databoard/list.do" class="btn btn-xs btn-info">목록</a></td>
					</tr>
					<tr id="delTr" style="display: none">
						<td colspan="4" class="text-right inline">비밀번호:<input
							type=password id="del_pwd" class="input-sm" size=10> <input
							type=button value="삭제" class="btn-warning btn-sm" id="deleteBtn">
						</td>
					</tr>
				</table>
				...
				<c:if test="${sessionScope.id!=null }">
					<table class="table">
						<tr>
							<td><textarea rows="4" cols="60" id="msg"
									style="float: left"></textarea> <input type="button"
								value="댓글쓰기"
