			    ...
			    = 데이터를 관리하는 프로그램 : 자바에서 출력 데이터를 전송 => 받아서 처리 
			      ========== 단일 데이터 : 문자열 / 여러개 데이터 ([],{})
						...
						자바는 오라클에 있는 데이터를 읽어와서 자바스크립트로 전송 
						      =======================================
						자바스크립트 데이터를 받아서 동적으로 화면 처리 => 데이터 변경 (태그 생성, 속성 선택, CSS제어, 이벤트 처리)
						HTML / CSS => 기본 화면 
			     ...
			     => 순서(동작)
			      ...
			      => 함수생성 방법
			         ...
			         3) 유형
			         		...
			         		============================
			         		          O          O
			         		  function func_name(id,pwd) => 매개변수명
			         		  {
			         		  			return값; => 리턴형을 사용하지 않는다
			         		  }
			         		============================
			         		           O         X
			         		 function func_name()
			        ...
			        = ES6 권장사항
			          ...
			          function func_name(){} => func_name("A")
			          let func_name=()=>{} => func_name("B")
			          let func_name=function(){} => func_name("C") => 데이터형은 매개변수값에 따라 변경
			                                        func_name(10)  // 넘어오는 값에 따라 데이터의 매개변수형이 달라진다 
			                                     => 문자열을 사용시 값에 '' , ""을 반드시 넣어야한다 
			     			** 내장객체
			     			** 태그 선택 방법
			     		 	= 자바스크립트 : 태그일긱 / 태그 생성 / 속성 읽기
			     		 	  태그 읽기
