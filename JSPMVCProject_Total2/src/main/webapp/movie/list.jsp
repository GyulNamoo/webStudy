		...
		JSP
		  1. 지시자
		  	 = page : contentType="" => 브라우저에 알려주는 방식
		  	          메모리에 저장된 데이터가 어떤 형식인지 알려준가 => response
		  	                      =============
		  	                      1) HTML 2) XML 3) JSON
		  	                      					 text/plain
		  	                      		    text/xml
		  	                      	text/html
		  	 = taglib : 태그명으로 자바의 제어문을 지원
		  	            prefix="개인에따라 지정 : c/fmt / sql/ xml/fn" uri="" 
		  	                      =======   import
		  	             <c:
		  	             ===
		  	    | 제어문 / 화면 이동
		  	      <c:if text="조건문">
		  	      <c:forEach var="받는변수" items="배열/컬렉션" varStatus="인데스번호">
		  	    ...
		  	    | EL : 데이터 출력
		  	      ...
		  	      ${sessionScope.키} => sesison.setAttribute()
		  	...
		  	===
			    ...
			    => View : 브라우저 출력용
			              ...
			              JSTL/EL
			    => 클래스 단위
			       메소드 단위 (**********) => djshxpdltus (ckwrl rlsmd)
