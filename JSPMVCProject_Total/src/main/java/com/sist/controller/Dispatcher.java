 ...
 *      = 장점
 *         1) 서블릿 => 자바코드만으로  HTML코드 작성 => 오류 발생할 확률이 낮다 
 ...
 *      => JSP/Servlet => 보완
 *         => 1. 확장성 / 재사용 증가
 *         => 2. 보안이 필요한 경우 처리 (HTML/Java의 완전 분리)
 *         => 3. 분업화 => Front-End / Back-End 
 *         => 4. Controller 로 집중적 => 서버에 부하가 많을 수 있다
 *               ========================================
 *               1) Front-End의 MVC => Redux / Flux / Vuex
 *               2) 서버를 나눠서 동시 구분 => MSA
 *                                      ==== 에자일 기법
 *           => 비용이 많이 들어간다
 *           => 목적은 유지 보수 목적 => 쉽게 수정 / 쉽게 추가ㅏ
 *           => 모든 편집기 => 어시스트
 *         => 공기업 / 금융권 / 대기업 : MVC구조로 만들어져 있다
 *            ===================  ======
 *                                  | => 스프링
 *            스프링 / 웹스페어 / 웹로직
 *            				   | 1대당 100만원
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			for(int i=0; i<strCls.length; i++)
			{
				...
				//클래스 메모리 할당
				// new =>결합성이 높다 (new사용을 자) => 스프링에서는 new를 사용하지 않는다
